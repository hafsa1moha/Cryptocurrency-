...
public class CryptoWalletAdapter extends RecyclerView.Adapter<CryptoWalletAdapter.Viewholder> {
    ...
    public void onBindViewHolder(@NonNull Viewholder holder, int position) {
        holder.cryptoNameTxt.setText(cryptoWallets.get(position).getCryptoNme());
        holder.cryptoPriceTxt.setText("$"+formatter.format(cryptoWallets.get(position).getPropertyAmount()));
        ...
        int drawableResourceId=holder.itemView.getContext().getResources()
                .getIdentifier(cryptoWallets.get(position).getCryptoNme(), "drawable", holder.itemView.getContext().getPackageName());
